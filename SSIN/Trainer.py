        ...
        try:
            ...
            for epoch in range(1, self.args.epochs+1):
                ...
                for data in train_data_iter:
                    ...
                    # 2. loss of predicting masked elements
                    per_example_loss = self.criterion(outputs, masked_labels).squeeze()  # [batch * max_pred]

                    # 雨點權重
                    with torch.no_grad():
                        high_rain_mask = (masked_labels.squeeze() > 0).float()
                        alpha = 2.0
                        importance = 1.0 + alpha * high_rain_mask 
                        
                    effective_weights = masked_label_weights.squeeze() * importance
                    numerator = torch.sum(per_example_loss * effective_weights)
                    denominator = torch.sum(effective_weights) + 1e-10
